...
const filterButtons = buttons.map((button) => `
  <button id="${button.id}" class="filterButtons">${button.text}</button>
  `
).join("")

buttonsDOM.innerHTML = filterButtons
...
buttonsDOM.addEventListener("click", (e) => 
{
  if (e.target.tagName === "BUTTON") 
  {
    ...
    const filteredItems = menu.filter((element) => element.category === ID || ID === "All");
    const filteredItemsDOM = filteredItems
      .map((element) => 
      `
      ...
      .join("");
    menuDOM.innerHTML = filteredItemsDOM;
  }
});